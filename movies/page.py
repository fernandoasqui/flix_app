...
def show_movies():
    ...
    selectted_actors_ids = [actor_names[name]for name in selected_actors_names]
    ...
    if st.button('Cadastrar'):
        new_movie = movie_service.create_movie(
            ...
            actors=selectted_actors_ids,
            resume=resume
        )
        if new_movie:
            st.rerun()
        else:
            st.erros('Erro ao cadastrar o filme. Verifique os campos.')
